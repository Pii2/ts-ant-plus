...
function updateState(
  ...
  let speedRevolutionCount = data.readUInt16LE(
  ...
  if (cadenceTime !== oldCadenceTime) {
    ...
    const cadence =
      (60 * (cadenceCount - oldCadenceCount) * 1024) /
      (cadenceTime - oldCadenceTime);
    if (!isNaN(cadence)) {
  ...
  if (speedEventTime !== oldSpeedTime) {
    ...
    const distance =
      sensor.wheelCircumference * (speedRevolutionCount - oldSpeedCount);
